...
# Combine features into a single text column for sentiment analysis
df['combined_features'] = df['Gaze_Status'] + " " + df['Expression'] + " " + df['Engagement'] + " " + df['Final_Status']

# Preprocess text using the simplified method
df['processed_features'] = df['combined_features'].apply(preprocess_text)

# Create binary target: 1 for attentive, 0 for not attentive
...
# Add NLTK sentiment scores with error handling
